...
module.exports = async (overTime, conf) => {
  try {
    ...
    */
     ...
     * @param {date} _nowDate 本日
     * @returns {number} 何週目かを示す数値
     */
    let weekNum = ((_nowDate) => {
      // 本日が何週目か単純に求める
      let val = Math.floor((_nowDate.date() - _nowDate.day() + 12) / 7);
      // 当月の1日が土日開始だった場合、報告資料のフォーマットに合わせて-1する
      let firstDay = moment([_nowDate.year(), _nowDate.month(), 1]);
      if (firstDay.day() === 6 || firstDay.day() === 0) val -= 1;
      // 今日が日曜 or 月曜なら、1週前にする
      if (_nowDate.day() === 0 || _nowDate.day() === 1) val -= 1;
      return val;
    })(nowDate);
    // 0以下の場合、1週目として扱う
