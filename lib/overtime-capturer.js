...
module.exports = async (userId, password) => {
  ...
  try {
    ...
    let elem = await driver.findElement(by.xpath('//small[text()="時間外ガイドライン（月間）"]/../../../following-sibling::tr'));
    elem = await elem.findElement(by.css('td > h3 > strong')); // td下の、残業時間取得
